...
test('online compatibility agrees with the pinned real Tab5 resolver on representative packages',()=>{
 ...
 const run=spawnSync(python,[require('node:path').join(__dirname,'fixtures/tab5-v3-resolver-m638.py')],{input:JSON.stringify(candidates.map(c=>c.package)),encoding:'utf8'});

...
function state(changes = {}) {
  return { schemaVersion: 4, kind: "well-pump-event-v3-runtime-state", packageVersion: 1, releaseId: "20260830123456-event-v3-v1", contentHash: "a".repeat(64), publishedAtMs: 1788266096000, deliveryEnabled: false, executionEnabled: true, ...changes };
}
...
test("Rules V3 state is closed, bounded, and truthfully execution-enabled", () => {
  ...
  for (const invalid of [state({ executionEnabled: false }), state({ unreviewed: true }), state({ deliveryEnabled: true }), state({ contentHash: "a".repeat(63) }), state({ packageVersion: 0 })]) {

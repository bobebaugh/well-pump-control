...
(async()=>{
 ...
 await page.click('#engine-load');await page.click('#load-saved');await page.waitForFunction(()=>state.draft!==null && !state.busy);
 assert.equal(memory.values.size,0);
 const original=defaults(),backup=makeBackup(original);
 await page.click('#engine-load');
 await page.setInputFiles('#backup-file',{name:'backup.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(backup))});
 await page.waitForFunction(()=>state.dirty.size===4 && !state.busy);
 assert.equal(memory.values.size,0);
 await page.click('#engine-validate');await page.waitForFunction(()=>!state.busy);
 assert.equal(memory.values.size,0,'validation leaves the saved store empty');
 await page.click('#engine-save');await page.waitForFunction(()=>!state.busy);
 ...
 assert.equal((await store.listReleases()).length,2,'deliberate unchanged publish creates a new version');
 await page.fill('#event-display-name','Revised title');failPublishResponse=true;
 await page.click('#engine-publish');await page.waitForFunction(()=>!state.busy);
 assert.equal((await store.listReleases()).length,3);assert.match(await page.textContent('#engine-status'),/Publication confirmed/);
 await page.click('#engine-deliver');await page.waitForFunction(()=>!state.busy);
 assert.equal((await store.listReleases()).length,3);

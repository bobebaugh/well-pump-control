...
**Status:** Hardware-qualified observational utility; pressure conversion not yet adopted by the normal application  
...
- Pressure transducer: nominal 0.5–4.5 V output for 0–100 PSI.
- ADS1110 mode: continuous.
- Sample rate: 15 SPS, 16-bit conversion.
- PGA: gain 2.
- M5 Unit ADC v1.1 terminal divider: nominal 6:1.
- ADC-pin full scale at gain 2: ±1.024 V.
- Terminal full scale at gain 2: ±6.144 V.
- Nominal terminal resolution: 187.5 µV/count.
...
- Displays S1–S5, arithmetic average, nominal sensor PSI, selected flow window, and signed derived GPM.
- Allows gauge PSI adjustment, direction metadata selection, and a 3–30 second flow window.
...
- Records S1–S5, arithmetic average, nominal PSI, actual measurement start/end/midpoint ticks, and elapsed time.
...
The zero test produced 62 complete CSV rows over 61.717 seconds. Sixty of 61 intervals were 952–1,061 ms and averaged 999.9 ms. One interval was 1.572 seconds. The delay was recorded correctly by the actual timestamps.

## Pressure calibration dataset and fit

The best-effort analog-gauge sweep contains nominal one-PSI observations from 60 down to 40 PSI, including two 40 PSI captures. The gauge has 10-PSI numbered marks with small one-PSI ticks; manual reading uncertainty is approximately ±0.3–0.5 PSI. The ADC is substantially more repeatable than the reference gauge:

- Within-batch spread during calibration: 1–5 counts.
...
The fitted conversion is not directly validated at 0 or 100 PSI. Readings near 66–70 PSI are modest extrapolations until physical evidence is obtained there.
...
- Observed running-power range: approximately 2,839–2,946 W.
...
- Calibrated 40–60 PSI rise time: approximately 89.5 seconds.

If house demand was isolated, the manufacturer's 20.5-gallon 40–60 PSI drawdown implies an average net tank-fill rate of approximately:
...
- 110 were 945–1,042 ms and averaged 999.8 ms.
...
3. The captured raw evidence supports an end-to-end linear pressure calibration over approximately 40–61 PSI.
4. The preferred current conversion is `PSI = (raw - 3732.02) / 211.492`.
5. The normal application's pressure conversion has not yet been changed to the fitted formula.

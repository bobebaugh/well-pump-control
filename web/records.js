...
const presets = [
  { id: "pump", label: "Pump & tank", columns: ["PressurePSI", "TankWaterGallons", "TankNetFlowGPM", "TankFlowQuality", "PumpWatts", "ContactorFlag", "PumpEnable"] },
...
const storageKey = "recordBrowserColumns.v1";
const state = { currentStart: null, currentDirection: "after", nextCursor: null, previousCursor: null, trail: [], columns: [], metadata: [], catalog: [], records: [], preset: "pump", eventColumns: null, customizeOpen: false, mode: new URLSearchParams(location.search).get("session") ? "session" : "observation", anchor: null };
...
// The selection is kept by name in this browser only. An event link shows that
// event's fields for the visit without overwriting what was saved.
...
function presetFor(columns) { const match = presets.find(item => item.columns.join(",") === columns.join(",")); return match ? match.id : state.eventColumns && state.eventColumns.join(",") === columns.join(",") ? "event" : "custom"; }
function present() { return new Set(state.catalog.map(item => item.name)); }
function shownColumns() { const names = present(); return state.columns.filter(name => names.has(name)); }
function renderColumns() {
  const names = present();
  const options = [...presets.map(item => [item.id, item.label]), ...(state.eventColumns?.length ? [["event", "Event fields"]] : []), ...(state.preset === "custom" ? [["custom", "Custom"]] : [])];
  const chips = state.columns.map(name => `<span class="column-chip${names.has(name) ? "" : " absent"}"${names.has(name) ? "" : ' title="Not in the records on this page"'}>${escapeHtml(name)}<button type="button" data-remove="${escapeHtml(name)}" aria-label="Remove ${escapeHtml(name)}">×</button></span>`).join("") || `<span class="column-empty">No field columns selected.</span>`;
  const offered = [...state.catalog.map(item => item.name), ...state.columns.filter(name => !names.has(name))].sort((left, right) => left.localeCompare(right));
  picker.innerHTML = `<div class="column-bar"><label>View<select data-kind="preset">${options.map(([id, label]) => `<option value="${id}"${id === state.preset ? " selected" : ""}>${label}</option>`).join("")}</select></label><div class="column-chips">${chips}</div></div>` +
    ...
    `<div class="column-options">${offered.map(name => `<label${names.has(name) ? "" : ' class="absent"'}><input type="checkbox" data-kind="data" value="${escapeHtml(name)}" ${state.columns.includes(name) ? "checked" : ""}>${escapeHtml(name)}</label>`).join("") || `<span class="column-empty">No fields in the records on this page.</span>`}</div></details>`;
}
...
picker.addEventListener("change", event => {
  ...
  if (kind === "preset") { const columns = target.value === "event" ? state.eventColumns || [] : presets.find(item => item.id === target.value)?.columns || state.columns; selectColumns(columns, target.value); load(state.currentStart, state.currentDirection); }
  ...
  else if (kind === "data") { selectColumns(state.columns.filter(name => name !== target.value)); renderColumns(); render(state.records); }
});
picker.addEventListener("click", event => { const name = event.target.dataset?.remove; if (name === undefined) return; selectColumns(state.columns.filter(item => item !== name)); renderColumns(); render(state.records); });
...
function leaveSession(mode) { state.mode = mode; state.anchor = null; state.currentStart = null; state.trail = []; }
